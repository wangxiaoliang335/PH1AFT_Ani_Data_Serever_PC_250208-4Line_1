		...
		if (theApp.m_PlcConectStatus)
		{
			...
			for (int ii = 0; ii < ChMaxCount; ii++)
			{
				...
				if (m_bStartFlag == FALSE)
				{
					...
					//theApp.m_PlcThread->LogWrite(CStringSupport::FormatString(_T("AOI DefectCode Panel %d Start Flag, eBitType_DefectCodeEnd1"), ii + 1));
				}
				else
				{
					theApp.m_PlcThread->LogWrite(CStringSupport::FormatString(_T("AOI DefectCode Panel %d Start Flag TRUE, eBitType_DefectCodeEnd1"), ii + 1));
				}
