...
void CAlignThread::TrayCheckNTrayAlignGrabMethod(int iPanelNum)
{
	...
	if(theApp.m_bPLCStopNStart == TRUE )
		sendMsg.Format(_T("%d,%s,%s,%s,%s,%s,%s"), MC_GRAB_READY_REQUEST, _T("1"), _T(""), _T(""), _T(""), _T(""), _T(""));
	else
		sendMsg.Format(_T("%d,%s,%s,%s,%s,%s,%s"), MC_GRAB_READY_REQUEST, _T("0"), _T(""), _T(""), _T(""), _T(""), _T(""));

	theApp.m_bPLCStopNStart = FALSE;
	theApp.m_AlignSocketManager[m_iAlignNum]->SocketSendto(m_iAlignNum, sendMsg, MC_GRAB_READY_REQUEST); 
