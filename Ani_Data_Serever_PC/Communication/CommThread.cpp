					...
					DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity)
{
	...
	if (m_hComm == (HANDLE) -1)
	{
		theApp.m_pFFUSendReceiverLog->LOG_INFO(_T("Port Open Fail"));
		//theApp.getMsgBox(MS_OK, _T("Port Open Fail"), _T("Port Open Fail"), _T("端口开启失败"));
		return FALSE;
	}
...
void CCommThread::OnDataReceive(WPARAM wParam, LPARAM lParam)
{
...
{
	//Port Window Handle //<<150704 JYLee
	CWnd* pWnd = (CWnd*)pComm;
   ...
   while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
   {
	   if (theApp.m_bExitFlag == FALSE)
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			...
			do
			{
				dwRead = pComm->ReadComm( buff, 8192); //들어온 데이터 읽어 오기 
...
//				else
//					AfxMessageBox("buff full"); //큐버퍼의 크기를 초과하면 경고 메시지 보냄  //>>121015.kty
			}while(dwRead);
			
			//ETX 가 있어야지만 전송해서 받을수 있도록 합니다. FFU 전용임		//20181129  hacy
			readMsg = pComm->m_QueueRead.buff;
			iCount = pComm->m_QueueRead.GetSize();
			for (int ii = 0; ii < iCount; ii++)
			{
				if (readMsg[iCount - 1] == _ETX)
				{
					pComm->OnDataReceive((WPARAM)&pComm->m_QueueRead.buff, LPARAM(pComm->m_QueueRead.GetSize()));
